...
if ( ! function_exists( 'mate_remove_version_from_style_js' ) ) {
...
if ( ! function_exists( 'mate_remove_recent_comments_style' ) ) {
...
if ( ! function_exists( 'mate_remove_type' ) ) {
    function mate_remove_type( $markup, $handle, $href ) {
    ...
    add_filter( 'script_loader_tag', 'mate_remove_type', 10, 3 );
    add_filter( 'style_loader_tag', 'mate_remove_type', 10, 3 );  // Ignore the $media argument to allow for a common function.
}

if ( ! function_exists( 'mate_wp_head_ob_start' ) ) {
    // Store and process wp_head output to operate on inline scripts and styles.
    function mate_wp_head_ob_start() {
        ob_start();
    }
    add_action( 'wp_head', 'mate_wp_head_ob_start', 0 );
}

if ( ! function_exists( 'mate_wp_head_ob_end' ) ) {
    function mate_wp_head_ob_end() {
    ...
    add_action( 'wp_head', 'mate_wp_head_ob_end', 10000 );
}